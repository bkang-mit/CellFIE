...
class Graph:
    ...
    def __init__(self, interactome_file, params=dict()):
        ...
        # This function determines edge_penalties
    ...
    def _reset_hyperparameters(self, params=dict()):
        ...
        Arguments:
            params (dict): params with which to run the program
        """
        # 083021 added another parameter, k
        defaults = {"w": 5, "b": 1, "g": 3, 'k': 1, "edge_noise": 0.1, "dummy_mode": "terminals", "seed": 0, "skip_checks": False}
        ...
        # add cell specific cost to each edge, proportional to the 1 - rank percentile, multiplied by k*edge_noise
        
        
        ## Calcualting cell specific cost and hub-node costs
        N = len(self.nodes)
        
        # Add cell specific cost
        #print(self.params.cell_type, 'testing reset_hyperparam')
        #self.cell_type = self.params.cell_type
        
        if self.params.cell_type != 'Naive':
            df = self.interactome_dataframe.copy()
            mycelltype = self.params.cell_type
            print(self.params.cell_type)
            df['adj_cost'] = df['cost'] + self.params.k * self.params.edge_noise * (1 - df[mycelltype+'_rank'])
            self.edge_costs = df['adj_cost'].astype(float).values
    ...
    def _random_scores(self):
        """
        Randomly assign scores to prize nodes
        Returns:
            numpy.array: new prizes with randomly assigned scores
        """
        # sorting by degree will be not necessary, as I will be only shuffling within prizes
        # this only selects prizes that are present in interactome (i.e. remove NA)
        prize_df = self.node_attributes[self.node_attributes.terminal==True]
        score_shuffled = prize_df.sample(frac=1).score.values
        prize_df_shuffled = prize_df.copy()
        prize_df_shuffled.score = score_shuffled # update with shuffled score 
        prize_df_shuffled.prize = prize_df_shuffled.raw_prize + prize_df_shuffled.score # update prize = raw_prize + score 
        # this score is already weighted by (5)
        
        
        
        # clip negative prize to 0
        prize_df_shuffled.loc[prize_df_shuffled.prize < 0, 'prize'] = 0
        #print(prize_df_shuffled[prize_df_shuffled.prize < 0])
        
        # check if prizes are actually shuffled
        if np.array_equal(prize_df.prize.values, prize_df_shuffled.prize.values):
            print("WARNING: Prize is not shuffled properly - make sure prizes are shuffled!")

        # update entire prize list, including Terminal==False
        shuffled_node_attributes = self.node_attributes.copy()
        shuffled_node_attributes.loc[prize_df.index] = prize_df_shuffled
        shuffled_bare_prizes = shuffled_node_attributes['prize'].values

        new_prizes = shuffled_bare_prizes * self.params.b
        old_prizes = self.bare_prizes
        if np.array_equal(new_prizes, old_prizes):
            print("WARNING: Prize is not shuffled properly")
    ...
    def _random_score_reps(self, reps):
        """
        ...
        true_prizes = self.prizes # held out original prizes
        
        for random_prizes in [self._random_scores() for rep in range(reps)]:
            self.prizes = random_prizes # update current prize with randomized prizes 
            results.append(self.pcsf())
        self.prizes = true_prizes # put the true prize back on
        
        return self._aggregate_pcsf(results, 'cell_specificity')
    ...
    def _eval_PCSF_runs(self, params):
        ...
        # 083021 adding k
        paramstring = "W_{:04.2f}_B_{:04.2f}_G_{:d}_K_{:d}".format(self.params.w, self.params.b, self.params.g, self.params.k)
    ...
    def grid_randomization(self, prize_file, cell_type, Ws=[5], Bs=[1], Gs=[3], Ks = [1], noisy_edges_reps=0, random_terminals_reps=0):
        ...
        #print('cell type update', cell_type)
        
        # 10/05/21 note that when accessing self.params.celltype, it throws an error:
        # TypeError: 'Options' object is not subscriptable
        #print(self.params['cell_type'])
        #celltype=self.params['cell_type']
        # also just using self.cell_type defined in reset_hyperparams function does not work..
        
        
        pool = multiprocessing.Pool(n_cpus)
        self.prepare_prizes(prize_file)

        print('testing input arguments')
        print('prize', prize_file)
        print('cell', cell_type)
        print(Ws, Bs, Gs, Ks)
        print(noisy_edges_reps, random_terminals_reps)
        
        # 083021
        param_sets = [{'w': w, 'b': b, 'g': g, 
                       'k': k, 
                       'noisy_edge_reps': noisy_edges_reps, 'random_terminals_reps': random_terminals_reps,
                       'cell_type': cell_type} for (w, b, g, k) in product(Ws, Bs, Gs, Ks)]
...
# This function may not be used at all
